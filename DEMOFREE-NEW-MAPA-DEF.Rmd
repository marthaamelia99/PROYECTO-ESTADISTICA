...
fviz_silhouette(res.agnes) #0.3 --> usare este
...
creo columna de cluster en la data
```{r}
set.seed(2019)
...
#creo data solo de cluster
hago la data de clusters 
```{r}
mapacluster <- data.frame(names = row.names(DEMOFREE), DEMOFREE)
mapacluster[,-c(1,45)]=NULL
...
```{r}
test=merge(mapaMundo, mapacluster, by.x='CNTRY_NAME', by.y='names')
```
...
mas demo ------------------------------------> menos demo
azul(4)    rosa(1)   verde(3)   amarillo(2)    gris(5)
