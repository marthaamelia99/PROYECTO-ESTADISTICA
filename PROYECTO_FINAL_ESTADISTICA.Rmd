#¿CUALES SON LOS FACTORES ESENCIALES PARA LA CALIDAD DE LA DEMOCRACIA?

#Ximena Amalia Andrade Rodríguez
#Alvaro Alejandro Herrada Ruiz
#Martha Amelia La Torre Frisancho
#Analourdes Román Morey Carrillo
#María del Carmen Venerio Acurio


EXTRACCION Y LIMPIEZA DE DATOS

#LLAMAR LA BASE DE DATOS A UTILIZAR, EN ESTE CASO LLAMAMOS LA DATA DEL HUMAN FREEDOM INDEX 
```{r}
library(rio)
...
#LIMPIEZA DEL HUMAN FREEDOM INDEX
```{r}
...
#LE CAMBIAMOS DE NOMBRE A CADA VARIABLE DEL HFI
```{r}
...
#CARGAR DATOS Y LIMPIEZA DE LA DATA DE DEMOCRACY INDEX
```{r}
library(rio)
```
...
#LIMPIEZA DE LA DATA DE DEMOCRACY INDEX 
```{r}
...
#LA NUEVA DATA SE LLAMA DEMOFREE
```{r}
str(DEMOFREE)
```
...
#CONSTRUCCION DE INDICES PROPUESTOS POR EL MARCO TEORICO 
```{r}
...
#NORMALIZACION DE LAS VARIABLES PARA LOS ANALISIS:LA MEDIA DE LAS VARIABLES PASA A SER 0 
```{r}
...
#AHORA PORCEDEMOS A HACER EL ANALISIS FACTORIAL EXPLORIO PARA COTEJAR LA CONSTRUCCION DE FACTORES "EN LA PRACTICA" QUE ARROJA EL PROGRAMA CON EL MODELO TEORICO PROPUESTO POR EL HFI  
...
#AHORA PROCEDEMOS A CALCULAR LA MATRIZ DE CORRELACION
```{r}
# esta es:
library(polycor)
...
```
#A TRAVES DE ESTOS GRAFICOS SE VER QUE HAY BLOQUES CORRELACIONADOS DE MODO QUE HAY ESPERANZA DE UN BUEN ANALISIS FACTORIAL 
...
#NO OBSTANTE CONTINUAMOS PUESTO QUE SOLO SE DESEA COMPARAR LO QUE NOS RECOMIENDE EL PROGRAMA -EL EFA- CON EL CFA BASADO EN EL MODELO TEORICO 
...
```
#NOS RECOMIENDA 3 FACTORES, NO OBSTANTE COMO SE DESEA CONTRASTAR CON EL MODELO TEORICO QUE PROPONE 6 LATENTES SE SEÑALARA QUE EL NUMERO DE FACTORES ES 6 
...
#SE ELABORAN LOS FACTORES EN BASE AL MODELO TEORICO PROVISTO POR EL HUMAN FREEDOM INDEX
```{r}
...
#PRUEBA DE KABLE PARA VISUALIZAR LA CONEXION DE LAS VARIABLES CON LOS FACTORES
```{r}
library(knitr)
...
```
#SE DETERMINA QUE NO TIENE BUEN AJUSTE PORQUE LA VARIANZA NO EXPLICADA ES MAYOR A 0.05
...
```
#VEMOS QUE NO HAY NORMALIDAD DE RESIDUOS PORQUE EL PVALUE ES MENOR A 0.05
...
#HACEMOS LA CLUSTERIZACIÓN EN BASE A LAS VARIABLES INDEPENDIENTES DE NUETSRO MODELO G: A PARTIR DE ELLO SE ELABORARÁ EL GRAFICO
```{r}
library(stringr)
library(magrittr)
library(htmltab)
...
fviz_silhouette(res.agnes) #0.3 --> se usara este
...
fviz_dend(res.agnes, cex = 0.3,horiz = T)
```
