...
```
#SI HAY MULTIICOLINEALIDAD EN ESTADO DE DERECHO, SECURITY AND SAFETY, Y EN IDENTIDAD Y RELAICONES 

#NO MULTICOLINEALIDAD DE LA I
```{r}
library(DescTools)
VIF(regI) # > 5 es problematico
```
...
#vemos que nos quedamos con la técnica jerárquica divisiva 
...
library(fpc)
library(ggrepel)
base + geom_text_repel(size=1,aes(label=row.names(inputData)))
```
```{r}
